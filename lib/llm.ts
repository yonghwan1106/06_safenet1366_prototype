...
[다국어 자동 감지]
- 사용자 발화의 주 언어를 감지해 같은 언어로 응답합니다 (한국어가 기본).
- 지원: 한국어, 영어(English), 베트남어(Tiếng Việt), 중국어(中文), 필리핀 영어/타갈로그.
- 다문화 가정 응답 시 한국어 표현 옆에 안내 전화번호 1366은 그대로 둡니다.

[안전계획 개인화]
- 컨텍스트로 가까운 보호시설 정보(이름·잔여 좌석·다국어 여부·전화)가 제공되면
  응답에 자연스럽게 한 문장으로 포함합니다 (강요·과시 금지).
- 시설 정보가 없으면 1366으로 통합 안내합니다.
...
export async function generateBotMessage(args: {
  ...
  const shelterHint = shelter
    ? `\n가까운 보호시설(시연용 가상 데이터): ${shelter.name} · 잔여 ${shelter.remaining}석 · ${shelter.multilingual ? '다국어 지원' : '국문'} · ${shelter.phone}`
    : '';
  const userMessage = `사용자가 챗봇에 입력한 발화입니다.

발화: ${utterance.slice(0, 500)}

자동 위험 평가: 등급 ${severity}/9, 권장 라우팅 = ${routing}${matchedHint}${shelterHint}

이 사용자에게 시스템 규약에 따른 응답을 작성해 주세요. 사용자 발화의 언어를 따라 응답합니다.`;
